import { Events, InteractionType, PermissionsBitField, GuildMember } from "discord.js";
...
module.exports = {
    ...
    async run(client: ICustomClient, interaction: any) {
        ...
        if (interaction.type === InteractionType.ApplicationCommand) {
            const command = client.commands.get(interaction.commandName) as ICommand | undefined;
            ...
            if (member && command.data.default_member_permissions) {
                ...
                if (!memberPermissions.has(command.data.default_member_permissions)) {
                    return await interaction.reply({
        ...
        if (interaction.type === InteractionType.MessageComponent) {
            ...
            if (interaction.isButton()) {
                ...
                const member = interaction.member as GuildMember | null;
                if (member && file.permission && !member.permissions.has(new PermissionsBitField(file.permission))) {
                    return await interaction.reply({
