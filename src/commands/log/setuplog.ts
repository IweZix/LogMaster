...
module.exports = {
    ...
    async run(interaction: any) {
        ...
        try {
            ...
            const guildData = fileData.find((data: any) => data.guildId === guildId);
            ...
            if (!guildData) {
                ...
                setupEmbed.setDescription(`The log channel has been set to ${logChannel.toString()}`);
                return await interaction.reply({ embeds: [setupEmbed] });

            } else {

                const oldLogChannel = interaction.guild.channels.cache.get(guildData.logChannelId);

                if (oldLogChannel === logChannel) {
                    return await interaction.reply("The log channel is already set to this channel!");
                }
